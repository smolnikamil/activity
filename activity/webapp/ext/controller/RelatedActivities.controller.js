sap.ui.define(
	...
	function (Controller, JSONModel, ExtensionAPI) {
		...
		return Controller.extend("fsm.connector.activity.ext.RelatedActivities", {
			/**
			 * Called when a controller is instantiated and its View controls (if available) are already created.
			 * Can be used to modify the View before it is displayed, to bind event handlers and do other one-time initialization.
			 * @memberOf fsm.connector.activity.ext.RelatedActivities
			 */
			onInit: function () {
			...
			_onRouteMatched: function (oEvent) {
				...
				if (oData.ActivityNumber !== "") {
					oModel.read(
						"/Activity('" + oData.ActivityNumber + "')/toActivityRelated",
						{
					...
					oJsonModel.setData({});
					this.getView().setModel(oJsonModel, "RelatedActivities");
				}
				...
				const setModelData = (nodes) => {
					...
					this.getView().setModel(oNodesModel, "RelatedActivities");
				};
					...
					if (nodesIn) {
						...
						for (var i = 0; i < nodesIn.length; i++) {
							...
							nodeOut = {
								...
								ActivityNumber: nodeIn.ActivityNumber,
								OrderNumber: nodeIn.OrderNumber,
								OperationNumber: nodeIn.OperationNumber,
								ActivityStatus: nodeIn.ActivityStatus,
								Person: nodeIn.Person,
								Subject: nodeIn.Subject,
								Technician: nodeIn.Technician,
								ActivityStartDate: nodeIn.ActivityStartDate,
								ActivityStartTime: nodeIn.ActivityStartTime,
								ActivityEndDate: nodeIn.ActivityEndDate,
								ActivityEndTime: nodeIn.ActivityEndTime,
								SourceActivity: nodeIn.SourceActivity,
								type: nodeIn.Type,
								children: [],
							};
